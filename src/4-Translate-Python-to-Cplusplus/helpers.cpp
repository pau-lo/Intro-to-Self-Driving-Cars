...
{
	float sum = 0;
	int lin = grid.size(),	// declaring a row == linha in portuguese
		col = grid[0].size(); // declaring a column == coluna in portuguese

	vector<vector<float>> newGrid(lin, vector<float>(col, 0));

	for (int i = 0; i < lin; i++) // i, j, k, l , or x, y, z;
	{
		for (int j = 0; j < col; j++)
		{
			sum = sum + grid[i][j];
...
{
	// declaring integers
	int alt = grid.size(),	 // altura
		larg = grid[0].size(); // largura

	// declaring floats
	float centProb = 1.0 - blurring, // getting the center probability
		cantoProb = blurring / 12.0, // getting the corner one
		adjaProb = blurring / 6.0;   // getting the adjacent one

	vector<vector<float>> window{{cantoProb, adjaProb, cantoProb},
								 {adjaProb, centProb, adjaProb},
								 {cantoProb, adjaProb, cantoProb}};

	vector<vector<float>> newGrid;

	newGrid.assign(alt, vector<float>(larg));
	for (int i = 0; i < alt; i++)
	{
